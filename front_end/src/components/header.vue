        ...
        <a-menu v-model="current" mode="horizontal" theme="light">
            ...
            <a-menu-item key="4" @click="selectMenu" v-if="userInfo.userType=='Manager'">
                <router-link :to="{ name: 'manageUser'}">
                     <a-icon type="user" />业务管理
                </router-link>
            </a-menu-item>
            <a-menu-item key="5" @click="jumpToVIPRegister" v-if="userInfo.userType=='Client'">
                <a-icon type="user" />业务办理
            </a-menu-item>
        </a-menu>
        <div class="logout">
            <a-dropdown placement="bottomCenter">
                <div class="user">
                    <a-avatar style="backgroundColor:#87d068" icon="user"></a-avatar>
...
export default {
    ...
    methods: {
        ...
        jumpToVIPRegister(){
            this.$router.push({name: 'vipRegister'})
        }
    }
}
</script>
