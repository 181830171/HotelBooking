<template>
    ...
    >
        <a-form :form="form" style="margin-top: 30px" v-bind="formItemLayout">
            ...
            <a-form-item label="管理员Id" v-bind="formItemLayout">
                <a-input
                        placeholder="请填写管理员Id，管理员必须已存在"
                        v-decorator="['managerId', { rules: [{ required: true, message: '请输入管理员Id' }] }]"
                />
            </a-form-item>
            ...
            <a-form-item label="酒店图片" v-bind="formItemLayout">
                <a-input
                        placeholder="请填写酒店图片链接"
                        v-decorator="['picture', { rules: [{ required: true, message: '请填写酒店图片链接' }] }]"
                />
            </a-form-item>
            ...
            <a-form-item label="酒店商圈" v-bind="formItemLayout">
                <a-select
                    ...
                    { rules: [{ required: true, message: '请选择酒店商圈' }] }]"
                        @change="changeStar"
                >
                    <a-select-option value="RegionA">A区</a-select-option>
                    <a-select-option value="RegionB">B区</a-select-option>
                    <a-select-option value="RegionC">C区</a-select-option>
                    <a-select-option value="RegionD">D区</a-select-option>
                    <a-select-option value="RegionE">E区</a-select-option>
                </a-select>
            </a-form-item>
            ...
            this.form.validateFieldsAndScroll((err, values) => {
                if (!err) {
                    const data = {
                        ...
                        managerId: Number(this.form.getFieldValue('managerId')),
                        picture: this.form.getFieldValue('picture'),
                        bizRegion: this.form.getFieldValue('BizRegion'),
                    }
                    this.set_addHotelParams(data)
                    this.addHotel(this.form.getFieldValue('BizRegion'))
                }
            });
        },
    }
